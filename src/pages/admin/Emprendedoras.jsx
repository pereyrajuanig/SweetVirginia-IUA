...
import {
  getEmprendedoras,
  cambiarEstadoEmprendedora,
} from "@/services/emprendedorasService";
import { Button } from "@/components/ui/button";
import Loader from "@/components/shared/Loader";
import { toast } from "sonner";
import FormularioEmprendedora from "@/components/shared/FormularioEmprendedora";
...
export default function Emprendedoras() {
  ...
  async function handleToggleSuspension(id, estadoActual) {
    ...
    await cambiarEstadoEmprendedora(
      id,
      nuevoEstado === "activa" ? "aprobada" : "rechazada",
    );
    setEmprendedoras((prev) =>
    ...
    toast(
      nuevoEstado === "activa"
        ? "Emprendedora reactivada."
        : "Emprendedora suspendida.",
      {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-[#A08070] text-sm mt-1">
            {emprendedoras.length} registradas en la plataforma
          </p>
        </div>
        <FormularioEmprendedora
          onEmprendedoraCreada={(nueva) =>
            setEmprendedoras((prev) => [nueva, ...prev])
          }
        />
      </div>
                ...
                {pendientes.map((e) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-[#A08070]">{e.telefono}</p>
                    </div>
            ...
            {resto.map((e, i) => (
              <div
                ...
                className={`flex items-center justify-between p-4 ${
                  i !== resto.length - 1 ? "border-b border-[#E8DDD6]" : ""
                }`}
              >
                <div>
                  <p className="font-medium text-[#3D2B1F]">
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`text-xs px-2.5 py-1 rounded-full font-medium ${badgeEstilo[e.estado]}`}
                  >
                    {e.estado}
                  </span>
                  {e.estado !== "pendiente" && (
