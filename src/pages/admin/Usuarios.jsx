        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-[#E8DDD6] overflow-hidden">
            ...
            {usuariosFiltrados.map((usuario, i) => (
              <div
                key={usuario.id}
                className={`flex flex-col sm:grid sm:grid-cols-4 items-start sm:items-center gap-2 sm:gap-0 px-4 py-4 ${
                  i !== usuariosFiltrados.length - 1
                    ? "border-b border-[#E8DDD6]"
                    : ""
                }`}
              >
                ...
                <p className="text-[#A08070] text-sm font-mono">
                  {usuario.telefono}
                </p>
                <div className="flex gap-1 flex-wrap">
                  <span
                    className={`text-xs px-2.5 py-1 rounded-full font-medium ${rolEstilo[usuario.rol]}`}
                  >
                    {usuario.rol}
                  </span>
                </div>
